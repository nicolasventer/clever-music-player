	...
	<Modal isOpen={isOpen} onClose={onClose} title="Songs Above Threshold" icon={<AlertTriangle size={20} />} closeOnClickOutside>
		<Vertical gap={16}>
						...
						{songs.map((song) => (
							...
							>
								<Horizontal justifyContent="space-between" alignItems="flex-start">
									...
									<Button icon={<Trash2 size={14} />} variant="filled" color="danger" onClick={TodoFn("delete song")} />
								</Horizontal>
							</div>
						))}
					</Vertical>
			...
			<Horizontal gap={12} justifyContent="center">
				{songs.length > 0 && (
					<Button icon={<Trash2 size={16} />} variant="filled" color="danger" onClick={TodoFn("delete all songs")}>
						Delete All
					</Button>
				)}
